...
function highlightTerm(term) {
  ...
  // Search for term in specific elements (adjust selector as needed)
  const elements = document.querySelectorAll('p'); // Or use a more generic selector
  ...
  elements.forEach(element => {
    ...
    if (index !== -1) {
      ...
      const node = element.childNodes[0]; // Assuming the text is in the first child node
...
document.getElementById("sendMessageButton").addEventListener("click", function() {
  ...
  window.location.href = "mailto:kolabdu.gmail.com?subject=Message from Website&body=Message body";
});